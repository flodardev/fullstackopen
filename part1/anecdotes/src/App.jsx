import { useState } from 'react'
import "./App.css"

const App = () => {
  const anecdotes = [
    'If it hurts, do it more often.',
    'Adding manpower to a late software project makes it later!',
    'The first 90 percent of the code accounts for the first 90 percent of the development time...The remaining 10 percent of the code accounts for the other 90 percent of the development time.',
    'Any fool can write code that a computer can understand. Good programmers write code that humans can understand.',
    'Premature optimization is the root of all evil.',
    'Debugging is twice as hard as writing the code in the first place. Therefore, if you write the code as cleverly as possible, you are, by definition, not smart enough to debug it.',
    'Programming without an extremely heavy use of console.log is same as if a doctor would refuse to use x-rays or blood tests when diagnosing patients.',
    'The only way to go fast, is to go well.'
  ]
   
  const anecdotesVotesArray = Array(anecdotes.length).fill(0)

  const [selected, setSelected] = useState(0)
  const [votes, setVotes] = useState(anecdotesVotesArray)
  const [highestVoteIndex, setIndex] = useState(0)

  const nextAnecdote = () => {
    let generateNewNumber = Math.floor(Math.random() * anecdotes.length)

    while (generateNewNumber === selected) {
      generateNewNumber = Math.floor(Math.random() * anecdotes.length)    
    }

    setSelected(generateNewNumber)
  }
  
  const getAnecdoteMostVotes = (copy) => {
    const highestVoteCount = Math.max(...copy)
    const indexOfHighestVoteCount = copy.indexOf(highestVoteCount)

    return indexOfHighestVoteCount
  }  


  const voteAnecdote = () => {
    const copy = [...votes]
    copy[selected] += 1
    setVotes(copy)

    setIndex(getAnecdoteMostVotes(copy))
  }



  return (
    <div>
      <h1>Anecdote of the day</h1>
      <div className="anecdotes">
        {anecdotes[selected]}
        <p>Vote count: {votes[selected]}</p>
      </div>

      <div>
        <AnecdoteVoteButton onClick={voteAnecdote}/>
        <NextAnecdoteButton onClick={nextAnecdote}/>
      </div>

      <h1>Anecdote with the most votes</h1>
      <div className="anecdotes">
        {anecdotes[highestVoteIndex]}
        <p>Vote count: {votes[highestVoteIndex]}</p>
      </div>
    </div>
  )
}

const NextAnecdoteButton = ({onClick}) => {
  return (
      <button onClick={onClick}>next anecdote</button>

  )
}

const AnecdoteVoteButton = ({onClick}) => {
  return (
      <button onClick={onClick}>Vote</button>
  )
}

export default App